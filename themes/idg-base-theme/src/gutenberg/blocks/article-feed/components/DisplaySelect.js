  ...
  getPreviewPosts() {
    ...
    const results = selectedPosts.map(resp => {
      ...
      const featuredImage = get(
        resp,
        "_embedded['wp:featuredmedia'][0].media_details.sizes['300-r3:2'].source_url",
      );
      ...
      if (resp.meta.multi_title) {
        let multiTitle = false;

        try {
          multiTitle = JSON.parse(resp.meta.multi_title);
        } catch (e) {
          //Silence
        }
      ...
      <div className="articleFeed-inner">
        {this.props.preview && (
          <SelectPreview
            ...
            displayEyebrows={this.props.displayEyebrows}
            displayExcerpt={this.props.displayExcerpt}
            displayBylines={this.props.displayBylines}
