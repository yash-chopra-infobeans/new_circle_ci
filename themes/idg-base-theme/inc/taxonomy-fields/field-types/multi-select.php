...
class Tax_Multi_Select extends Tax_Field_Template {
	...
	public function edit_field() {
		...
		foreach ( $this->data['options'] as $option ) {
			...
			printf('<option value="%s" %s>%s</option>', esc_attr( $option_val ), in_array( $option_val, $this->data['value'], true) ? 'selected' : '', esc_html( $option->name ) );
		}
